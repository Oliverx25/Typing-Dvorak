...
function FingerSlot({
  ...
  return (
    ...
    >
      ...
      />
      <span className={['text-[9px] sm:text-[10px]', active ? 'font-semibold text-[var(--color-key-target)]' : 'text-[var(--color-text-muted)]'].join(' ')}>
        {label}
      </span>
    </div>
  );
  ...
  return (
    <div className="mt-4 flex items-end justify-center gap-6 sm:gap-10">
      <div className="flex flex-col items-center gap-2">
        <span className="text-[10px] uppercase tracking-widest text-[var(--color-text-muted)]">L</span>
        <div className="flex items-end gap-1 sm:gap-2">
          {left.map(({ finger, label }) => (
            <FingerSlot key={finger} finger={finger} label={label} active={activeFinger === finger} />
          ))}
        </div>
        <div className="h-3 w-24 rounded-full bg-[var(--color-border)] sm:w-28" aria-hidden="true" />
      </div>

      <div className="flex flex-col items-center gap-2">
        <span className="text-[10px] uppercase tracking-widest text-[var(--color-text-muted)]">R</span>
        <div className="flex items-end gap-1 sm:gap-2">
